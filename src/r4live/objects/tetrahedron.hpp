...
class Tetrahedron : public Entity<VecType> {
    __host__ __device__ Tetrahedron (const Array<Vec4d, 4> &points)
            : points{points} {}
    ...
    __host__ __device__ bool hit(const Ray<VecType> &ray,
        ...
        if (space_norm.dot(ray.direction())) {
            return false;
        }
        ...
        auto is_inside = same_sides(points[0], points[1], points[2], points[3], P, space_norm) &&
